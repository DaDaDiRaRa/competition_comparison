...
const s = {
  ...
  modal: {
    background: '#1a1f2e', borderRadius: 14, width: '90%', maxWidth: 920,
    maxHeight: '90vh', display: 'flex', flexDirection: 'column',
    border: '1px solid #2d3748', boxShadow: '0 20px 60px rgba(0,0,0,0.6)',
  },
  header: {
    padding: '18px 24px', borderBottom: '1px solid #2d3748',
    display: 'flex', alignItems: 'center', gap: 12,
  },
  title: { fontSize: 16, fontWeight: 700, color: '#e2e8f0', flex: 1 },
  closeBtn: {
  ...
  sidebar: {
    width: 160, borderRight: '1px solid #2d3748',
  ...
  sideBtn: (active) => ({
    ...
    background: active ? '#2b4c7e' : 'transparent',
    color: active ? '#90cdf4' : '#718096',
    marginBottom: 2,
  }),
  content: { flex: 1, overflowY: 'auto', padding: 24 },
  footer: {
    padding: '14px 24px', borderTop: '1px solid #2d3748',
    display: 'flex', gap: 10, justifyContent: 'flex-end', alignItems: 'center',
  },
  cancelBtn: {
    background: 'transparent', border: '1px solid #4a5568', borderRadius: 8,
    color: '#a0aec0', padding: '9px 20px', cursor: 'pointer', fontSize: 14,
  },
  saveBtn: (dirty) => ({
    background: dirty ? '#2f855a' : '#1a2e1a',
    border: 'none', borderRadius: 8, color: dirty ? '#fff' : '#4a5568',
    padding: '9px 24px', cursor: dirty ? 'pointer' : 'not-allowed',
    fontSize: 14, fontWeight: 700, transition: 'all 0.15s',
  }),
  label: { fontSize: 12, color: '#a0aec0', marginBottom: 5, display: 'block' },
  input: {
    background: '#0d1117', border: '1px solid #2d3748', borderRadius: 6,
    padding: '8px 10px', color: '#e2e8f0', fontSize: 13, width: '100%',
  ...
  sectionTitle: { fontSize: 14, fontWeight: 700, color: '#90cdf4', marginBottom: 16 },
  original: { fontSize: 11, color: '#4a5568', marginTop: 3 },
  tag: {
    display: 'inline-flex', alignItems: 'center', gap: 5,
    background: '#1a2e40', border: '1px solid #2c5282', borderRadius: 20,
    padding: '3px 10px', fontSize: 12, color: '#90cdf4', margin: '3px 3px 3px 0',
  },
  tagRemove: {
    background: 'none', border: 'none', color: '#718096',
  ...
  toast: (type) => ({
    fontSize: 13, color: type === 'ok' ? '#68d391' : '#fc8181',
    flex: 1,
  }),
}
          ...
          {RESULT_OPTIONS.map(opt => (
            <div
              ...
              style={{
                ...
                border: result === opt.value ? `2px solid ${opt.color}` : '2px solid #2d3748',
                background: result === opt.value ? opt.bg : '#0d1117',
      ...
      {rows.map((row, i) => (
        <div key={i} style={s.rowCard}>
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: '#a0aec0', fontWeight: 600 }}>층 {i + 1}</span>
...
function AdvancedSection({ extracted, onChange, onErrorChange }) {
  ...
  return (
    <div>
      ...
      <textarea
        style={{ ...s.textarea, minHeight: 300, borderColor: error ? '#fc8181' : '#2d3748' }}
        value={text}
        onChange={e => handleChange(e.target.value)}
      />
      {error && <div style={{ fontSize: 12, color: '#fc8181', marginTop: 4 }}>{error}</div>}
    </div>
  )
}
  ...
  if (loading) return (
    <div style={s.overlay}>
      <div style={{ ...s.modal, alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
        <div style={{ color: '#a0aec0', fontSize: 14 }}>데이터 불러오는 중...</div>
      </div>
    </div>
  )
  ...
  return (
    <div style={s.overlay} onClick={e => { if (e.target === e.currentTarget) handleClose() }}>
      <div style={s.modal}>
        ...
        <div style={s.body}>
          {loadError ? (
            <div style={{ padding: 40, color: '#fc8181', fontSize: 14, lineHeight: 1.7 }}>
