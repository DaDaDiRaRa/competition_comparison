...
async def extract_pdf(
    ...
    # ── BRIEF_DESIGN_*: 순차 실행 + 직전 페이지 컨텍스트 주입 (시퀀스 D-B) ──────
    # 스태킹(A안)은 이미지 해상도 페널티(3페이지 압축 → 페이지당 1/3 해상도)와
    # 출력 토큰 한도 압박으로 자식 항목 누락 + facility_scope="전체" 평탄화 발생.
    # 대안: 같은 type 연속 페이지를 그룹핑하고, 그룹 안에서는 순차 실행하면서
    # 직전 페이지의 마지막 grouped 항목(facility_scope/space_scope/section_path)을
    # 다음 페이지 프롬프트에 컨텍스트 힌트로 주입.
    # 페이지별 풀 해상도 유지 + 부모-자식 계층 보존 동시 달성.
    # 다른 type 그룹끼리는 병렬 (BRIEF_DESIGN_GUIDE / MASSING / SUSTAIN / SPECIAL).
    design_groups: list[list[tuple[int, str]]] = []
    if is_brief:
        _sorted_design = sorted(
            pg for pg, pt in type_by_page.items() if pt in _DESIGN_BRIEF_TYPES
        )
        _current_group: list[tuple[int, str]] = []
        _prev_pg: int | None = None
        _prev_type: str | None = None
        for _pg in _sorted_design:
            _pt = type_by_page.get(_pg)
            if (
                _current_group
                and _pt == _prev_type
                and _prev_pg is not None
                and _pg == _prev_pg + 1
            ):
                _current_group.append((_pg, _pt))
            else:
                if _current_group:
                    design_groups.append(_current_group)
                _current_group = [(_pg, _pt)]
            _prev_pg = _pg
            _prev_type = _pt
        if _current_group:
            design_groups.append(_current_group)

    precomputed_design: dict[int, dict] = {}

    async def _process_design_group(group: list[tuple[int, str]]) -> None:
        """한 type 그룹 내 페이지들을 순차 실행 (컨텍스트 누적)."""
        _prev_ctx: str | None = None
        for _pg, _pt in group:
            _img = pages_by_num.get(_pg)
            if not _img:
                continue
            _base_cfg = EXTRACTION_PROMPTS_BRIEF.get(_pt)
            if not _base_cfg:
                continue
            if _prev_ctx:
                _ctx_note = (
                    "\n\n[직전 페이지 컨텍스트 — 같은 type의 직전 페이지에서 마지막에 추출된 항목들]\n"
                    + _prev_ctx
                    + "\n[규칙: 이 페이지 상단에 새 굵은 헤더(예: '4-X. xxx', 시설명, 큰 글자 섹션 제목)가 "
                    "보이지 않으면 위 facility_scope / space_scope / section_path 컨텍스트를 그대로 "
                    "이어받아 사용. 새 헤더가 보이면 새 컨텍스트로 시작. 존재하지 않는 가공의 상위 "
                    "헤더(예: '외부시설 계획지침')를 만들지 말 것.]"
                )
                _cfg = {**_base_cfg, "instruction": _base_cfg["instruction"] + _ctx_note}
            else:
                _cfg = _base_cfg
            try:
                _result = await asyncio.to_thread(_extract_page_sync, _img, _pg, _pt, _cfg)
            except Exception as _e:
                _result = {"page": _pg, "type": _pt, "data": {"error": str(_e)}}
            precomputed_design[_pg] = _result
            # 컨텍스트 업데이트: 결과의 마지막 2~3개 grouped 항목
            _grouped = (_result.get("data") or {}).get("design_guidelines_grouped") or []
            if _grouped:
                _last = _grouped[-3:]
                _lines = []
                for _it in _last:
                    _lines.append(
                        f"- facility_scope: {_it.get('facility_scope', '') or '전체'} | "
                        f"space_scope: {_it.get('space_scope', '') or '전체'} | "
                        f"section_path: {_it.get('section_path', '') or '(없음)'}"
                    )
                _prev_ctx = "\n".join(_lines)

    if design_groups:
        await asyncio.gather(*[_process_design_group(g) for g in design_groups])
    ...
    async def extract_one(img_bytes: bytes, page_num: int) -> dict:
        ...
        # BRIEF_DESIGN_*: 위에서 그룹 순차 실행으로 미리 추출된 결과 반환
        if page_num in precomputed_design:
            return precomputed_design[page_num]
