...
def _limits_determined_by(special_conditions: list) -> str:
    ...
    ⚠ **판정 조건(`_LIMIT_KW`)을 넓히려면 arch-law-diagnose 에 먼저 알려야 한다**
    (2026-08-28 상호 확인). 그쪽은 이 값을 `review_premised` 상태로 받아 **상한(max)에만**
    적용한다 — 주차 같은 법정 **최소**는 심의로 면제되지 않기 때문이고, 그 전제가
    「우리가 건폐율·용적률·높이만 본다」는 것이다. 한쪽이 조건을 넓히면 다른 쪽이
    **조용히 어긋난다**(예: 여기에 '주차'를 더하면 그쪽이 법정 최소를 심의 전제로
    완화해 버린다). 그쪽 코드에도 같은 주석이 박혀 있다.

    그쪽이 실제로 읽는 필드는 이 값 + `required_parking_count`·`parking_note` + 사업규모
    3종뿐이다. 블록의 다른 필드는 우리 소비처(5곳) 기준으로 자유롭게 넣고 뺀다 —
    **의미가 바뀔 때만** 통지 대상이다.

    이 필드가 잡아낸 실제 사고(2026-08-28 그쪽 라이브 e2e): 영등포 용적률 460%(심의
    상향분)를 준공업 법정 400% 와 그냥 견주니 종합판단이 **「패스 권장」**, 즉 앱이
    "응찰하지 마라"고 말하고 있었다. 심의 전제로 읽으니 「협상 필요」로 바뀐다.
    """
