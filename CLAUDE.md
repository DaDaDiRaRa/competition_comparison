...
**다음 작업 — '변수' 프로토타입:** 변수 = 정답이 하나가 아닌 지점(변수-1 공공성 해석, 변수-2 관점·테마 부여)에서 **사용자가 LLM 과 대화를 반복하며 컨셉을 조종**하는 루프. 원칙: A층 사실은 절대 고정, 해석층만 사용자 지시(steering)를 프롬프트에 주입해 재생성 — `/interpret`·`/propose` 패턴(추출 재처리 0, LLM 1콜)의 확장이 자연 구현 경로. 메모리 `project_byeonsu_conversational_steering` 참조.
