...
- **시퀀스 F — 대지·맥락 분석 통합 (제안서 깊이의 진짜 레버):** ✅ 기본 구현 완료 (2026-06-26, `services/vworld_analyzer.py`, `POST /api/brief/{id}/site-analyze`). 제안서 payload에 `_site_context` 자동 주입. VWorld 키는 `VWORLD_API_KEY` 환경변수 고정 (서버 고정값, per-user 불필요). ⚠️ **미해결 — VWorld 도메인 불일치:** vworld.kr 키 신청 시 등록한 도메인과 실제 Cloud Run URL(`...a.run.app`)이 달라 배포 환경에서 WMS 호출 시 도메인 오류 가능. 해결: vworld.kr 키 관리 페이지에서 Cloud Run 도메인 추가 등록 후 `VWORLD_DOMAIN` env var 설정. 로컬 테스트는 `localhost` 등록으로 동작. **나머지 보류:** ② 지적도/현황측량 실제 형상 ③ SketchUp MCP 3D 매스 — 사용자가 자료 줄 때 재개. ⚠️ **교훈:** 공간 데이터 없이 추론하면 틀린 전제를 세움 — 실제로 위성 확인 전 "강변 리버뷰"로 오추론했다가 위성 판독으로 "산을 낀 도심 부지"로 정정. 따라서 대지 자료(위성·지적도·현황도) 확보 전엔 대지 주장에 **추론/확인필요 라벨 필수**. **자료 조달:** ① **VWorld(국토부 공간정보 오픈플랫폼)** = 1순위 — 위성/항공영상 + **연속지적도** + 지오코딩 **무료**, WMS `https://api.vworld.kr/req/wms`. ② 보조: 네이버 Static Map / 카카오 로컬. 한계: 연속지적도=근사(측량성과 아님)·영상 촬영시점 고정. **1회 수동이면 지도 캡처 붙여넣기가 더 빠름(키 0); API는 자동화·앱기능화용.**
