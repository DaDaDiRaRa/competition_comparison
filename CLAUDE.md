...
- **HTML Comparison Report:** Generated at end of compare pipeline using existing comparison JSON — no extra Claude API calls. Includes: submission cards, 8-axis comparison table with score bars and compliance tags (지침충족/부분충족/미충족), ranking, key differentiators, winner strength analysis. Winner entries highlighted in gold. 대시보드 기본 펼침 축은 `business_viability`.
