...
- `services/data_extractor.py` - Extracts structured design data from pages; `merge_extracted_data()` returns `_quantitative` dict at top level
  ...
  - **`_extract_docx_block_with_llm` max_out 계층화:** `BRIEF_PROGRAM`/`BRIEF_EVALUATION` → 8000토큰, `BRIEF_DESIGN_MASSING`/`BRIEF_DESIGN_GUIDE`/`BRIEF_DESIGN_FACADE`/`BRIEF_DESIGN_SUSTAIN`/`BRIEF_DESIGN_SPECIAL` → 4000토큰, 기타 → 2000토큰. F3 force-cut(`_FORCE_CUT_PARAS=30`)으로 블록당 최대 30단락이 보장되므로 4000이면 가장 빽빽한 설계지침 블록도 충분히 커버.
