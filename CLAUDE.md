...
| `brief_contradiction.py` | 지침서 **내부 모순** 탐지 (LLM 0 · 값 수정 0) — 같은 양을 여러 곳이 다르게 말하면 flag. `detect_contradictions(brief_data)` → `_contradictions`. **계기(실제 사고)**: `_quantitative.site_area_sqm` 이 대지면적이 아니라 **부지1의 연면적 합계**(56,189.72)를 들고 있었고 체크리스트 핵심수치 폴백 사슬 끝이 그 값이라 **5.4배 틀린 대지면적**이 리포트 첫 화면에 떴다(실제 10,438㎡ · prod 21건 중 **7건** · 2026-06~08). 같은 지침서 안에 옳은 값(`feasibility_export.sites[]`)이 있었는데 아무도 안 물었다. ⚠**「함의 용적률(연면적÷대지) vs 명시 용적률」은 안 쓴다** — 연면적이 지하를 포함해 문턱이 필요한데 prod 표본이 사실상 한 공모뿐이라 보정 불가고, 다부지면 명시 용적률이 단일 숫자가 아니다(「부지1: 460% / 부지2: 400%」). **소스 불일치는 문턱 보정이 필요 없다.** 면적(가산 가능·다부지는 **합**)만 본다 — 건폐율·용적률은 부지별로 **정당하게 달라** 단일값과 대조하면 헛경고. **어느 쪽이 맞는지 안 정한다**(원문 확인 필요). 렌더: `band_html`(체크리스트 HTML, 본문 앞) · `md_lines`(md·HWPX §0.3). 회귀: `tests/test_brief_contradiction.py` (22). |
...
**`_brief.json` 의 `_contradictions` (지침서 내부 모순, 모순 있을 때만):** `[{quantity, label, sources: [{where, value}], spread_ratio}]` — 같은 양(총 대지면적·총 연면적)을 여러 곳이 다르게 말할 때. `spread_ratio`=최대÷최소(「5.38배」가 「56,189 vs 10,438」보다 빨리 읽힌다). **값 수정 0 · 어느 쪽이 맞는지 판정 0.** 면적만 본다(백분율은 다부지에서 정당하게 다름).
...
- [x] ~~**B-2 모순 탐지**~~ ✅ 2026-08-28 — `services/brief_contradiction.py`. 「이미 있을 것」이라 의심했는데 **아니었다** — `quant_validator` 는 `if quant and not is_brief` 로 **지침서를 명시적으로 제외**한다(지침서 `_quantitative` 는 키가 2개뿐이라 건폐율 항등식이 성립할 재료가 없다). 착수해서 **라이브 결함을 찾았다**(위 Core Services 행). 곁가지로 폴백 사슬도 고쳤다 — `_extract_sections` 의 대지면적이 `_quantitative` 보다 **`feasibility_export.sites[]` 합**을 먼저 보게(7/21 → 0/21).
...
- [x] ~~**D-1 SVG 겹침·잘림 회귀**~~ ✅ 2026-08-28 — `tests/test_svg_layout.py`(7). concept-studio `render/qa.py` 의
      **기법만** 가져왔다(모노 메트릭 라틴 0.6em·한글 1em, 브라우저 0 — 「크롬이 필요하면 CI 에서 skip 되고,
      skip 되는 검사는 없는 검사다」). **런타임 코드 0 · 테스트 전용.**
      착수 전 실측이 설계를 바꿨다: prod 21건에서 우리 SVG 세 계열이 **전부 깨끗**하다(매스 19 텍스트 ·
      면적 스택 12 · 제안서 16). 그쪽이 만들자마자 6건 잡은 것과 다른 이유는 **구조 차이** — 우리 조닝
      다이어그램은 라벨을 SVG 안이 아니라 **HTML 카드**(`pz-loc`·범례)에 둔다(SVG 30개에 `<text>` 16개뿐).
      그래서 「찾아내는 기능」으로는 값이 0이고(A-3 폐기와 같은 이유) **회귀 가드**로만 값이 있다 —
      `brief_massing` 은 라벨 19개를 SVG 안에 그리고 아직 손대는 중이다(정북 계단컷 미반영).
      ⚠**검사기가 본 것의 수(`n_svg`·`n_txt`)를 함께 확인한다** — 아무것도 안 본 검사기는 언제나 깨끗하다고
      말한다(픽스처 스키마를 지어냈다가 실제로 이 가드에 걸렸다).
      ⚠**못 하는 것**: 도형끼리 겹침 · 글자↔도형 겹침 · 실제 폰트 메트릭 · 대비 · 빈 영역.
